...
const MovieDetail = ({ getMovieDetail, movie, match, history, removeLastMovie }) => {
    useEffect(() => {
        if (!movie) {
        ...
        <Grid container style={{ textAlign: 'center' }}>
            <Grid item xs={8} sm={3}>
                <div style={{ border: '1px solid #000', backgroundColor: '#eff5ef', margin: '5px', padding: '10px ' }}>
                    ...
                    <hr />
                    <div>
                        <button onClick={clearCurrentMovie} className='btn btn-primary'type='submit'>Back</button>
                    </div>
                </div>
            </Grid>
        </Grid>
